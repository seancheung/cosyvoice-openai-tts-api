...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        log.exception("failed to load CosyVoice model")
        raise
    yield


app = FastAPI(title="CosyVoice OpenAI-TTS API", version="1.0.0", lifespan=lifespan)
...
async def healthz(request: Request) -> HealthResponse:
    ...
    if engine is None:
        return HealthResponse(status="loading", version=settings.cosyvoice_version)
...
async def preview_voice(id: str, request: Request):
    ...
    return FileResponse(path=str(voice.wav_path), media_type="audio/wav", filename=f"{id}.wav")
...
async def create_speech(body: SpeechRequest, request: Request):
    ...
    if body.response_format not in CONTENT_TYPES:
        raise HTTPException(status_code=422, detail=f"unsupported response_format: {body.response_format}")
    ...
    try:
        tensor = await engine.synthesize(
            tts_text=text,
    ...
    try:
        audio_bytes, content_type = encode(tensor, engine.sample_rate, body.response_format)
    except Exception as e:
