...
def _to_mono_float32(tensor: torch.Tensor) -> np.ndarray:
...
def encode(tensor: torch.Tensor, sample_rate: int, fmt: str) -> Tuple[bytes, str]:
    fmt = fmt.lower()
    if fmt not in CONTENT_TYPES:
        raise ValueError(f"unsupported response_format: {fmt}")

    samples = _to_mono_float32(tensor)
